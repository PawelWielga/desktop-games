...
describe("paper soccer logic", () => {
  ...
  it("declares the blocked player as the loser", () => {
    const state: PaperSoccerState = {
      ...
      visitedPoints: ["1:1"],
      segments: [
        { from: { x: 1, y: 2 }, to: { x: 0, y: 1 }, player: "away" },
        { from: { x: 1, y: 2 }, to: { x: 0, y: 2 }, player: "away" },
        { from: { x: 1, y: 2 }, to: { x: 0, y: 3 }, player: "away" },
        { from: { x: 1, y: 2 }, to: { x: 1, y: 3 }, player: "away" },
        { from: { x: 1, y: 2 }, to: { x: 2, y: 1 }, player: "away" },
        { from: { x: 1, y: 2 }, to: { x: 2, y: 2 }, player: "away" },
        { from: { x: 1, y: 2 }, to: { x: 2, y: 3 }, player: "away" },
      ],
      winner: null,
    };
